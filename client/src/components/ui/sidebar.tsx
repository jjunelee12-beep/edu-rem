    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh w-full bg-transparent text-slate-900",
            className
          )}
          {...props}
        >
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "flex h-full w-(--sidebar-width) flex-col bg-transparent text-slate-900",
          className
        )}
        {...props}
      >
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          side={side}
          className="w-(--sidebar-width) border-r-0 bg-transparent p-0 text-slate-900 [&>button]:hidden"
          style={
            {
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex flex-col gap-2 px-3 pb-2 pt-3", className)}
      {...props}
    />
  );
}
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn("flex flex-col gap-2 px-3 pb-3 pt-2", className)}
      {...props}
    />
  );
}
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-slate-500 outline-hidden transition-transform hover:bg-white hover:text-slate-900 focus-visible:ring-2",
        "after:absolute after:-inset-2 md:after:hidden",
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute right-2 top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-slate-200 px-1 text-xs font-medium tabular-nums text-slate-700 select-none",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mt-1 ml-2 flex min-w-0 flex-col gap-1 rounded-2xl bg-white p-2 shadow-[inset_0_0_0_1px_rgba(15,23,42,0.04)]",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
