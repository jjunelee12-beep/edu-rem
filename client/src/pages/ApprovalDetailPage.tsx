...
import {
  getFieldSetting,
  type ApprovalFieldSetting,
} from "@/lib/approvalFieldSettings";
...
export default function ApprovalDetailPage() {
  ...
  const fieldSettingsQuery = trpc.approval.getFieldSettings.useQuery(
    {
      formType: (doc?.formType || "general") as
        | "attendance"
        | "business_trip"
        | "general",
    },
  ...
  const detailField = getFieldSetting(
    fieldSettings,
    "attendanceDetailType",
    "상세 유형"
  );
  const startField = getFieldSetting(
    fieldSettings,
    "attendanceStartTime",
    "시작 시간"
  );
  const endField = getFieldSetting(
    fieldSettings,
    "attendanceEndTime",
    "종료 시간"
  );

  const destinationField = getFieldSetting(
    fieldSettings,
    "destination",
    "목적지"
  );
  const visitPlaceField = getFieldSetting(
    fieldSettings,
    "visitPlace",
    "방문처"
  );
  const companionField = getFieldSetting(
    fieldSettings,
    "companion",
    "동행자"
  );

  const deptField = getFieldSetting(
    fieldSettings,
    "requestDepartment",
    "요청 부서"
  );
          ...
          <div className="grid gap-3 md:grid-cols-2">
              ...
              <>
                {detailField.isVisible ? (
                  <DetailRow
                    ...
                    value={doc.attendanceDetailType}
                  />
                ) : null}
                  ...
                  <DetailRow
                    ...
                    value={
                      doc.attendanceStartTime
                        ? formatDateTime(doc.attendanceStartTime)
                        : "-"
                    }
                  />
                ) : null}
                  ...
                  <DetailRow
                    ...
                    value={
                      doc.attendanceEndTime
                        ? formatDateTime(doc.attendanceEndTime)
                        : "-"
                    }
                  />
                ) : null}
            ...
            {doc.formType === "business_trip" && (
              <>
                ...
                {visitPlaceField.isVisible ? (
                  <DetailRow
                    label={visitPlaceField.label}
                    value={doc.visitPlace}
                  />
                ) : null}
            ...
            {deptField.isVisible ? (
              <DetailRow
                ...
                value={doc.requestDepartment}
              />
            ) : null}

            {noteField.isVisible ? (
              <DetailRow label={noteField.label} value={doc.extraNote} />
            ) : null}
          </div>
          ...
          {[...lines]
            ...
            .map((line: any) => {
              ...
              return (
                <div
                  key={line.id}
                  className={`rounded-xl border p-4 ${getLineCardClass(
                    line,
                    doc
                  )}`}
                >
