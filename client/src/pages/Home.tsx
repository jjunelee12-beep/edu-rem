...
import { formatTime } from "@/lib/dateimport { useMemo, useState, useEffect } from "react";
import { useAuth } from "@/_core/hooks/useAuth";
import { useLocation } from "wouter";
import { trpc } from "@/lib/trpc";
import { toast } from "sonner";
                  ...
                  <CardContent className="p-5">
                    ...
                    {isSuperAdmin || isTeamManager ? (
                      <>
                        ...
                        <div className="home-attendance-list mt-4 space-y-2">
                          {visibleAttendanceList.length === 0 ? (
                            <div className="rounded-2xl bg-[#eef1f6] px-3 py-3 text-sm text-muted-foreground">
                              오늘 등록된 근태 기록이 없습니다.
                            </div>
                          ) : (
                            visibleAttendanceList.map((item) => (
                              <div
                                key={item.id}
                                className="flex items-center justify-between rounded-2xl bg-[#f5f6fa] px-3 py-2"
                              >
                                <div className="flex items-center gap-2">
                                  {item.status === "출근" ? (
                        ...
                        <div className="rounded-2xl bg-slate-50 p-4">
                          ...
                          <p className="mt-1 text-sm font-semibold">
                            {myTodayAttendance?.workMinutes
                              ? `${Math.floor(Number(myTodayAttendance.workMinutes) / 60)}시간 ${Number(
                                  myTodayAttendance.workMinutes
                                ) % 60}분`
                              : "-"}
                          </p>
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="space-y-4">
                      {urgentNotices.slice(0, 3).map((notice: any) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <p className="truncate font-semibold text-red-700">🚨 {notice.title}</p>
                      ...
                      {pinnedNotices.slice(0, 3).map((notice: any) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <p className="truncate font-semibold text-amber-800">📌 {notice.title}</p>
                            <span className="text-xs text-muted-foreground">
                              {notice.createdAt
                      ...
                      <div className="rounded-2xl bg-[#f5f6fa] p-4">
                        ...
                        <div className="mt-4 space-y-2">
                          {selectedCalendarDateSchedules.length === 0 ? (
                            <div className="rounded-xl bg-[#eef1f6] px-3 py-3 text-sm text-muted-foreground">
                              선택한 날짜에 등록된 일정이 없습니다.
                            </div>
                          ) : (
                            selectedCalendarDateSchedules.slice(0, 3).map((item: any) => (
                              <div key={item.id} className="rounded-xl bg-[#eef1f6] px-3 py-3">
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="space-y-3">
                        {(todaySchedules as any[]).map((item: any) => (
                          <div key={item.id} className="rounded-2xl bg-[#f5f6fa] px-4 py-3">
                            <div className="flex items-center justify-between gap-3">
                              <div>
