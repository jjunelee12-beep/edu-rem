...
export default function SemesterList() {
  ...
  const statusBadge = (sem: any) => {
    if (sem.isCompleted) return <Badge className="bg-emerald-100 text-emerald-700 text-[10px]">결제완료</Badge>;
    if (sem.actualPaymentDate) return <Badge className="bg-blue-100 text-blue-700 text-[10px]">결제중</Badge>;
    return <Badge className="bg-amber-100 text-amber-700 text-[10px]">예정</Badge>;
  };

  // 합계 계산
  const totalPlanned = filtered.reduce((sum: number, s: any) => sum + Number(s.plannedAmount || 0), 0);
  const totalActual = filtered.reduce((sum: number, s: any) => sum + (s.isCompleted ? Number(s.actualAmount || 0) : 0), 0);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-0 shadow-sm">
          <CardContent className="pt-4 pb-3 px-4">
            ...
            <p className="text-xl font-bold">{filtered.length}<span className="text-sm font-normal text-muted-foreground ml-1">건</span></p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="pt-4 pb-3 px-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/50 border-b">
                <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[90px]">예정개강월</th>
                <th className="text-center px-2 py-2.5 font-medium text-muted-foreground w-[50px]">학기</th>
                <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[80px]">학생명</th>
                <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[110px]">연락처</th>
                <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[120px]">과정</th>
                {isAdmin && <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[60px]">담당자</th>}
                <th className="text-left px-3 py-2.5 font-medium text-muted-foreground w-[90px]">교육원(예정)</th>
                <th className="text-right px-3 py-2.5 font-medium text-muted-foreground w-[100px]">예정금액</th>
                <th className="text-center px-2 py-2.5 font-medium text-muted-foreground w-[50px]">과목수</th>
            ...
            <tbody>
              {filtered.map((sem: any) => (
                <tr key={sem.id} className="border-b hover:bg-muted/20 cursor-pointer" onClick={() => setLocation(`/students/${sem.studentId}`)}>
                  ...
                  <td className="px-3 py-2 font-medium text-sm text-primary hover:underline">{sem.clientName}</td>
                  <td className="px-3 py-2 text-muted-foreground text-sm">{sem.phone || "-"}</td>
                  <td className="px-3 py-2 text-sm">{sem.course || "-"}</td>
                  {isAdmin && <td className="px-3 py-2 text-xs text-muted-foreground">{userMap.get(sem.assigneeId) || "-"}</td>}
                  ...
                  <td className="px-3 py-2 text-center">
                    {sem.hasPractice ? (
                      <Badge className={
                        ...
                        sem.practiceStatus === "섭외중" ? "bg-blue-100 text-blue-700 text-[10px]" :
                  ...
                  <td className="px-3 py-2 text-center">
                    <Badge className={
                      sem.approvalStatus === "승인" ? "bg-emerald-100 text-emerald-700 text-[10px]" :
                      sem.approvalStatus === "불승인" ? "bg-red-100 text-red-700 text-[10px]" :
              ...
              {!filtered.length && (
                <tr><td colSpan={isAdmin ? 12 : 11} className="text-center py-8 text-muted-foreground text-sm">
                  {plannedMonth ? `${plannedMonth} 예정 학기가 없습니다.` : "학기 데이터가 없습니다."}
                </td></tr>
              )}
            </tbody>
          </table>
