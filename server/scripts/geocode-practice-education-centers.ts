...
const BATCH_SIZE = Number(process.env.GEOCODE_BATCH_SIZE || 300);
const CONCURRENCY = Number(process.env.GEOCODE_CONCURRENCY || 3);
const REQUEST_DELAY_MS = Number(process.env.GEOCODE_DELAY_MS || 180);

const FAIL_LOG_PATH = path.resolve(
  process.cwd(),
  "geocode-practice-education-centers-failed.json"
);
...
function buildCandidateQueries(row: EducationCenterRow): string[] {
...
async function geocodeWithFallback(row: EducationCenterRow): Promise<{
  lat: number;
  lng: number;
  matchedQuery: string;
} | null> {
...
async function loadRows(conn: mysql.Connection, limit: number): Promise<EducationCenterRow[]> {
  const [rows] = await conn.query(
    ...
    SELECT
      ...
      address,
      detailAddress
    FROM practice_education_centers
    WHERE isActive = 1
  ...
  return rows as EducationCenterRow[];
}
...
async function updateRow(
  ...
  await conn.query(
    `
    UPDATE practice_education_centers
    SET
      latitude = ?,
      longitude = ?,
      geocodedAt = NOW()
...
async function worker(conn: mysql.Connection, rows: EducationCenterRow[], workerNo: number) {
  let success = 0;
  let fail = 0;
...
async function main() {
  ...
  try {
    ...
    if (!rows.length) {
      console.log("지오코딩할 교육원 대상이 없습니다.");
      return;
    }

    console.log(`교육원 대상 ${rows.length}건 시작`);
    console.log(`동시성=${CONCURRENCY}, 딜레이=${REQUEST_DELAY_MS}ms`);

    const chunks: EducationCenterRow[][] = Array.from({ length: CONCURRENCY }, () => []);
