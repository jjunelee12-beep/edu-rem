...
- 특히 현재 Stage에 "상세 설명" transition과 "다음 단계 진행" transition이 함께 존재하는 경우,
  사용자의 짧은 긍정 응답을 불필요하게 null로 처리하지 않는다.

- 직전 assistant가 현재 단계의 기본 안내를 마친 뒤
  상세 설명 또는 다음 단계 진행을 제안했고,
  사용자가 "네", "예", "넵", "응", "좋아요", "계속해주세요",
  "다음이요", "그다음요", "이어주세요"처럼
  별도의 상세 항목을 지정하지 않고 긍정 또는 계속 진행 의사를 표현하면
  "다음 상담단계로 진행"하는 transition을 우선 선택한다.
...
- 반대로 사용자가
  ...
  "행정절차 신청방법 알려주세요",
  "그 부분 더 설명해주세요"
  처럼 현재 단계의 세부정보를 명시적으로 요청한 경우에만
  상세 설명 transition을 선택한다.

- 따라서 단순한 긍정 응답만으로 상세 설명 transition을 선택하지 않는다.
  짧은 긍정 응답은 최근 assistant 발화가 현재 단계 설명 완료 후
  다음 진행을 제안한 문맥이라면 다음 Stage 진행 의사로 해석한다.
