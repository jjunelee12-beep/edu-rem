...
export type KakaoAiCapability =
   ...
   * 담당자 연결을 진행하려는 요청.
   *
   * 자연어 표현 자체는 Intent AI가 판단한다.
   *
   * 예:
   * - 상담 신청 의사
   * - 담당자 연결 의사
   * - 현재 상담내용을 CRM 상담DB에 접수하려는 의사
   */
  | "lead_registration"

  /**
   * 담당자의 전화상담 / 콜백을 희망하는 요청.
   *
   * AI는 사용자의 자연어 의도를 판단하고,
   * 실제 희망일/희망시간 저장 및 실행 여부는
   * 서버 Action이 검증한다.
   */
  | "callback_request";
...
const LEAD_ALLOWED_CAPABILITIES =
  new Set<KakaoAiCapability>([
    ...
    /**
     * 신규 상담자는 상담 중 선택한 담당자에게
     * 실제 상담 접수를 요청할 수 있다.
     *
     * 상담DB 생성 여부는 별도 서버 Action이
     * 필수정보와 담당자 선택상태를 검증한 뒤 결정한다.
     */
    "lead_registration",

    /**
     * 신규 상담자는 담당자에게
     * 전화상담 / 콜백 희망을 전달할 수 있다.
     *
     * AI가 예약 완료를 임의로 확정하지 않고
     * 서버에 실제 저장된 결과만 완료로 표현한다.
     */
    "callback_request",
  ]);
...
export const KAKAO_AI_CORE_CONVERSATION_RULES = [
  ...
  "사용자가 담당자 연결, 상담접수, 전화상담 또는 콜백을 자연어로 요청하면 표현 자체를 정해진 문장과 비교하지 말고 Intent Router가 현재 대화문맥을 이용하여 실제 의도를 판단한다.",

  "상담DB 접수, 담당자 연결, 전화상담 희망일 또는 희망시간 저장 등 실제 시스템 변경이 필요한 요청은 반드시 해당 서버 Action의 실행결과를 기준으로 답변한다.",

  "서버 Action이 실제로 성공하지 않은 상태에서는 '접수했습니다', '등록했습니다', '예약했습니다', '진행해두겠습니다', '연결했습니다'처럼 실제 처리가 완료되었다고 오해할 수 있는 표현을 사용하지 않는다.",

  "실제 전화예약 기능이 존재하지 않고 상담DB에 통화 희망일 또는 희망시간만 저장하는 구조라면 이를 '예약 완료'라고 표현하지 않고 담당자에게 통화 희망내용을 전달하거나 기록했다고 정확하게 안내한다.",
  /**
   * =========================================================
   * 상담 범위
   * =========================================================
